...
class ApexWebphone {
    ...
    showClickToCallRinging(destination) {
        ...
        this.updateCallCard({
            title: 'Outgoing call',
            subtitle: normalized
                ? `Ringing ${normalized}… your outbound number is shown below.`
        ...
        this.updateFloatingPopup({
            title: 'Outgoing call',
            subtitle: normalized
                ? `Ringing ${normalized}… destination will see your outbound caller ID.`
                : 'Ringing destination… waiting for answer.',
            detail: routeDetail,
            visible: true,
            statusLabel: 'Ringing',
    ...
    async pollDestinationOnce() {
        ...
        try {
            ...
            if (!data.live && data.hangup_cause) {
                ...
                const cause = String(data.hangup_cause).replace(/_/g, ' ').toLowerCase();
                showToast(
                    `Destination leg ended (${cause}). If the phone never rang, check Morpheus trunk routing.`,
                    'warning',
                );
            }
        } catch {
