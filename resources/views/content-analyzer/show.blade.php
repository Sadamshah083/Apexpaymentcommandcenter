...
@section('content')
    <div class="mb-6">
        <a href="{{ request()->is('admin*') ? route('admin.content.index') : route('portal.content.index') }}"
    ...
    <div class="grid md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-xl shadow-sm border p-6 text-center">
            ...
            <p
                class="text-4xl font-bold {{ ($result['mail_tester_score'] ?? 0) >= 7 ? 'text-green-600' : (($result['mail_tester_score'] ?? 0) >= 4 ? 'text-amber-600' : 'text-red-600') }}">
            ...
            @foreach ($result['scores'] ?? [] as $category => $score)
                @if ($score != 0)
                    <div class="flex justify-between py-1 text-sm border-b">
                        ...
                        <span
                            class="font-mono {{ $score > 0 ? 'text-red-600' : 'text-green-600' }}">{{ $score > 0 ? '+' : '' }}{{ $score }}</span>
                    </div>
                @endif
            @endforeach
        </div>
