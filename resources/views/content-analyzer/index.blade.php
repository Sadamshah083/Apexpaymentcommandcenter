            ...
            <table>
                ...
                <tbody>
                    @forelse($analyses as $analysis)
                        <tr>
                            <td>
                                <div class="font-medium text-slate-800">{{ $analysis->title ?? $analysis->subject }}</div>
                                @if($analysis->title && $analysis->subject)
                                    <div class="text-xs text-slate-500 mt-0.5">{{ Str::limit($analysis->subject, 60) }}</div>
                                @endif
                            </td>
                            <td>
                                <span class="px-2 py-0.5 rounded text-xs font-semibold {{ $analysis->overall_score >= 7 ? 'bg-rose-100 text-rose-800' : ($analysis->overall_score >= 4 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800') }}">
                                    {{ $analysis->overall_score }}/10
                                </span>
                            </td>
                            <td class="text-slate-500">{{ $analysis->created_at->diffForHumans() }}</td>
                            <td class="text-right">
                                <a href="{{ request()->is('admin*') ? route('admin.content.show', $analysis) : route('portal.content.show', $analysis) }}" class="text-indigo-600 hover:text-indigo-800 font-semibold text-sm">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
