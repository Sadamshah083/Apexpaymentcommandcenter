...
@section('content')
    ...
    <div class="grid md:grid-cols-3 gap-4 mb-8">
        <div class="bg-white rounded-xl shadow-sm border p-5">
            ...
            <p class="text-2xl font-bold {{ $hygiene['avg_invalid_rate'] > 5 ? 'text-red-600' : 'text-green-600' }}">
                {{ $hygiene['avg_invalid_rate'] }}%</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border p-5">
    ...
    <div class="grid md:grid-cols-2 gap-6 mb-8">
        <div class="bg-white rounded-xl shadow-sm border p-6">
            ...
            <p class="text-xs text-slate-500 mt-4"><a href="https://postmaster.google.com/" target="_blank"
                    ...
                    href="https://support.google.com/mail/answer/81126" target="_blank" class="text-indigo-600">Sender
                    Guidelines</a></p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
            ...
            <form action="{{ route($routePrefix . 'reputation.log') }}" method="POST" class="space-y-3">
                ...
                <input type="text" name="value" placeholder="e.g. 0.05% or HIGH"
                    class="w-full border rounded px-3 py-2 text-sm">
    ...
    <div class="app-data-table mb-8">
        ...
        <div class="app-data-table-footer">
            <p id="warmup-summary" class="app-pagination-summary">Showing first 2 weeks of {{ count($warmupSchedule) }}
                days ramping to {{ number_format($warmupTarget) }}/day.</p>
        </div>
    </div>
    ...
    <x-data-table title="Reputation Logs" :paginator="$logs">
        <table>
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-center py-8 text-slate-500">No logs yet. Paste weekly data from
                            Google Postmaster Tools.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
