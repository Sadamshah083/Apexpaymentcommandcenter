...
<div class="grid grid-cols-2 md:grid-cols-5 gap-4">
    ...
    <div class="app-card app-card-padded">
        ...
        <p class="text-3xl font-bold mt-1 {{ $stats['pending_jobs'] > 0 ? 'text-amber-600' : 'text-zinc-900' }}">{{ $stats['pending_jobs'] }}</p>
...
<div class="grid md:grid-cols-3 gap-6">
    <div class="app-card app-card-padded md:col-span-2">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('admin.workflows.create') }}" class="text-xs font-bold text-zinc-500 hover:text-zinc-900 underline">New Pipeline</a>
        </div>
        <div class="space-y-3">
            @forelse($recentWorkflows as $workflow)
                <a href="{{ route('admin.workflows.show', $workflow) }}" class="block p-3 bg-cream-50 hover:bg-cream-200 border border-warmgrey-200 rounded-xl transition-all">
                    <div class="flex justify-between items-center">
                        <span class="font-bold text-warmgrey-900 text-sm">{{ $workflow->name }}</span>
                        <span class="text-[10px] px-2 py-0.5 rounded-full font-extrabold uppercase tracking-wider
                    ...
                    <span class="text-xs text-warmgrey-500 block mt-1">
                        {{ $workflow->processed_leads }} / {{ $workflow->total_leads }} processed · {{ $workflow->original_filename }}
                    </span>
                </a>
            @empty
                <div class="text-center py-6 bg-cream-50 rounded-xl border border-dashed border-warmgrey-200">
                    <p class="text-warmgrey-500 text-sm">No active pipelines. <a href="{{ route('admin.workflows.create') }}" class="text-warmgrey-900 underline font-bold">Upload spreadsheet</a> to start.</p>
                </div>
            @endforelse
        </div>
    </div>
    ...
    <div class="app-hero flex flex-col justify-between">
        ...
        <a href="{{ route('admin.workflows.index') }}" class="app-btn app-btn-secondary bg-white text-zinc-900 hover:bg-zinc-100 block text-center mt-6">
            Open command center
        </a>
    </div>
</div>
...
<div class="grid md:grid-cols-3 gap-6">
    <div class="app-card app-card-padded">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('admin.lists.create') }}" class="text-xs font-bold text-zinc-500 hover:text-zinc-900 underline">Upload</a>
        </div>
        <div class="space-y-2">
            @forelse($recentLists as $list)
                <a href="{{ route('admin.lists.show', $list) }}" class="block p-2 hover:bg-zinc-50 rounded-lg text-xs font-semibold text-zinc-900 transition-colors">
                    <span>{{ $list->name }}</span>
                    <span class="text-[10px] text-zinc-500 block mt-0.5">{{ $list->total_count }} emails · {{ $list->status }}</span>
                </a>
            @empty
    ...
    <div class="app-card app-card-padded">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('admin.deliverability.index') }}" class="text-xs font-bold text-zinc-500 hover:text-zinc-900 underline">New Test</a>
        </div>
        <div class="space-y-2">
            @forelse($recentDeliverability as $test)
                <a href="{{ route('admin.deliverability.show', $test) }}" class="block p-2 hover:bg-zinc-50 rounded-lg text-xs font-semibold text-zinc-900 transition-colors">
                    <span>{{ $test->domain }}</span>
                    <span class="text-[10px] text-zinc-500 block mt-0.5">Score: {{ $test->overall_score }}/10</span>
                </a>
            @empty
    ...
    <div class="app-card app-card-padded">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('admin.content.index') }}" class="text-xs font-bold text-zinc-500 hover:text-zinc-900 underline">Analyze</a>
        </div>
        <div class="space-y-2">
            @forelse($recentContent as $content)
                <a href="{{ route('admin.content.show', $content) }}" class="block p-2 hover:bg-zinc-50 rounded-lg text-xs font-semibold text-zinc-900 transition-colors">
                    <span>{{ $content->title ?? 'Analysis' }}</span>
                    <span class="text-[10px] text-zinc-500 block mt-0.5">Spam risk: {{ $content->overall_score }}/10</span>
                </a>
            @empty
...
<div class="app-card app-card-padded bg-zinc-50">
    ...
    <ol class="text-xs text-zinc-500 space-y-2 list-decimal list-inside leading-relaxed">
        <li>Sync disposable domains helper: <code class="bg-white px-2 py-0.5 border border-zinc-200 rounded text-zinc-900">php artisan email-checker:sync-disposable-domains --sync</code></li>
        <li>Start parallel queue workers for faster lead enrichment:
            <code class="bg-white px-2 py-0.5 border border-zinc-200 rounded text-zinc-900">php artisan queue:pool</code>
            <span class="text-zinc-500">(default: 2 workers — set <code class="text-xs">QUEUE_WORKERS=4</code> in <code class="text-xs">.env</code> for 4× parallel)</span>
        </li>
