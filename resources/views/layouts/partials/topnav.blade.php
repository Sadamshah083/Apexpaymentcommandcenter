...
@php
    ...
    $initials = strtoupper(
        substr($nameParts[0] ?? 'U', 0, 1) . (isset($nameParts[1]) ? substr($nameParts[1], 0, 1) : ''),
    );
@endphp
...
<header class="app-topnav">
    ...
    <div class="app-topnav-right">
        @if ($workspace)
            <details class="app-topnav-dropdown">
                <summary class="app-topnav-btn app-topnav-btn-workspace">
                    <svg class="app-topnav-btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
                            d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0H5m14 0h-2m-12 0H3m2-16h10M9 7h1m-1 4h1m4-4h1m-1 4h1" />
                    </svg>
                    <span class="app-topnav-btn-label">{{ $workspace->name }}</span>
                    <svg class="app-topnav-btn-chevron" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        aria-hidden="true">
                    ...
                    @forelse($switchableWorkspaces as $ws)
                        ...
                        @else
                            <form method="POST" action="{{ route($workspaceSwitchRoute, $ws->id) }}">
                                @csrf
                                <button type="submit"
                                    class="app-topnav-panel-item app-topnav-panel-item-btn">{{ $ws->name }}</button>
                            </form>
                        @endif
                    @empty
        ...
        <button type="button" id="push-btn" class="app-topnav-icon-btn" onclick="requestPushNotificationPermission()"
            aria-label="System notifications" title="System notifications">
        ...
        <details class="app-topnav-dropdown">
            ...
            <div class="app-topnav-panel app-topnav-panel-user">
                ...
                <form method="POST" action="{{ $logoutRoute }}">
                    @csrf
                    <button type="submit"
                        class="app-topnav-panel-item app-topnav-panel-item-btn app-topnav-panel-signout">Sign
                        out</button>
                </form>
            </div>
        </details>
