...
@section('content')
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-500">Workspace: <span class="font-semibold text-slate-700">{{ $workspace->name }}</span> ·
                Live performance tracking and lead metrics.</p>
        </div>
        <div
            class="flex items-center gap-2 bg-indigo-50 text-indigo-700 border border-indigo-200 px-3 py-1.5 rounded-full text-xs font-semibold">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="divide-y divide-slate-100">
                <a href="{{ route('admin.workflows.index') }}"
                    class="flex justify-between py-2.5 hover:bg-slate-50 px-2 rounded-lg transition group">
                    <span class="text-slate-500 font-medium group-hover:text-indigo-600">Total Leads</span>
                    <span id="stat-total_leads"
                        class="font-extrabold text-slate-800 text-lg">{{ $pipeline['total_leads'] }}</span>
                </a>
                <a href="{{ route('admin.workflows.index', ['phase' => 'imported']) }}"
                    class="flex justify-between py-2.5 hover:bg-slate-50 px-2 rounded-lg transition group">
                ...
                <a href="{{ route('admin.workflows.index', ['phase' => 'enriched']) }}"
                    class="flex justify-between py-2.5 hover:bg-slate-50 px-2 rounded-lg transition group">
                    <span class="text-slate-500 font-medium group-hover:text-indigo-600">Qualified</span>
                    <span id="stat-qualified"
                        class="font-extrabold text-slate-800 text-lg">{{ $pipeline['qualified'] }}</span>
                </a>
                <a href="{{ route('admin.workflows.index', ['phase' => 'appointment_settled']) }}"
                    class="flex justify-between py-2.5 hover:bg-slate-50 px-2 rounded-lg transition group">
                ...
                <a href="{{ route('admin.workflows.index', ['phase' => 'with_closer']) }}"
                    class="flex justify-between py-2.5 hover:bg-slate-50 px-2 rounded-lg transition group">
                ...
                <a href="{{ route('admin.workflows.index', ['phase' => 'closed']) }}"
                    class="flex justify-between py-2.5 hover:bg-slate-50 px-2 rounded-lg transition group">
                    <span class="text-slate-500 font-medium group-hover:text-green-600">Closed (Won)</span>
                    <span id="stat-closed_won"
                        class="font-extrabold text-green-600 text-lg">{{ $pipeline['closed_won'] }}</span>
                </a>
                <a href="{{ route('admin.workflows.index', ['phase' => 'with_setter']) }}"
                    class="flex justify-between py-2.5 hover:bg-slate-50 px-2 rounded-lg transition group">
                ...
                <a href="{{ route('admin.workflows.index', ['phase' => 'closed', 'search' => 'closed_lost']) }}"
                    class="flex justify-between py-2.5 hover:bg-slate-50 px-2 rounded-lg transition group">
            ...
            <div>
                ...
                <div class="divide-y divide-slate-100">
                    <div class="flex justify-between py-3 px-2">
                        ...
                        <span id="stat-book_to_show"
                            class="font-extrabold text-slate-800 text-lg">{{ $conversion_rates['book_to_show_rate'] !== null ? $conversion_rates['book_to_show_rate'] . '%' : '-' }}</span>
                    </div>
                    <div class="flex justify-between py-3 px-2">
                        <span class="text-slate-500 font-medium">Show &rarr; Close Rate</span>
                        <span id="stat-show_to_close"
                            class="font-extrabold text-slate-800 text-lg">{{ $conversion_rates['show_to_close_rate'] !== null ? $conversion_rates['show_to_close_rate'] . '%' : '-' }}</span>
                    </div>
                    <div class="flex justify-between py-3 px-2">
                        <span class="text-slate-500 font-medium">Overall Close Rate</span>
                        <span id="stat-overall_close"
                            class="font-extrabold text-green-600 text-lg">{{ $conversion_rates['overall_close_rate'] !== null ? $conversion_rates['overall_close_rate'] . '%' : '-' }}</span>
                    </div>
                    <div class="flex justify-between py-3 px-2">
                        <span class="text-slate-500 font-medium">Avg Closed Deal Volume</span>
                        <span id="stat-avg_deal_volume"
                            class="font-extrabold text-slate-800 text-lg">${{ number_format($conversion_rates['avg_closed_volume'], 2) }}</span>
                    </div>
                    <div class="flex justify-between py-3 px-2">
                        <span class="text-slate-500 font-medium">Total Team Dials (period)</span>
                        <span id="stat-total_dials"
                            class="font-extrabold text-slate-800 text-lg">{{ $conversion_rates['total_dials'] }}</span>
                    </div>
                    <div class="flex justify-between py-3 px-2">
                        <span class="text-slate-500 font-medium">Total Team Closes (period)</span>
                        <span id="stat-total_closes"
                            class="font-extrabold text-slate-800 text-lg">{{ $conversion_rates['total_closes'] }}</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <tbody id="setters-table-body" class="divide-y divide-slate-50 text-sm text-slate-600">
                        @forelse ($setters as $setter)
                            <tr class="hover:bg-slate-50 transition cursor-pointer"
                                onclick="window.location='{{ route('admin.workflows.index', ['assigned_user_id' => $setter['id']]) }}'">
                                <td class="py-3.5 font-medium text-slate-800">
                                    <a href="{{ route('admin.workflows.index', ['assigned_user_id' => $setter['id']]) }}"
                                        class="text-indigo-600 hover:text-indigo-900">{{ $setter['name'] }}</a>
                                </td>
                                <td class="py-3.5 text-right font-extrabold text-slate-800">{{ $setter['leads_logged'] }}
                                </td>
                            </tr>
                        @empty
                            <tr>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="closers-table-body" class="divide-y divide-slate-50 text-sm text-slate-600">
                        @forelse ($closers as $closer)
                            <tr class="hover:bg-slate-50 transition cursor-pointer"
                                onclick="window.location='{{ route('admin.workflows.index', ['assigned_user_id' => $closer['id']]) }}'">
                                <td class="py-3.5 font-medium text-slate-800">
                                    <a href="{{ route('admin.workflows.index', ['assigned_user_id' => $closer['id']]) }}"
                                        class="text-indigo-600 hover:text-indigo-900">{{ $closer['name'] }}</a>
                                </td>
                                <td class="py-3.5 text-right font-extrabold text-green-600">{{ $closer['deals_closed'] }}
                                </td>
                            </tr>
                        @empty
                            <tr>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody id="workflows-table-body" class="divide-y divide-slate-50 text-sm text-slate-600">
                    @forelse ($workflows as $wf)
                        <tr class="hover:bg-slate-50 transition">
                            <td class="py-3.5 font-semibold text-slate-800">
                                <a href="{{ route('admin.workflows.show', $wf['id']) }}"
                                    class="text-indigo-600 hover:text-indigo-900">{{ $wf['name'] }}</a>
                            ...
                            <td class="py-3.5 text-right">
                                <a href="{{ route('admin.workflows.show', $wf['id']) }}"
                                    class="app-btn app-btn-secondary app-btn-sm !py-1 !px-2">
                                    Track file
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="py-4 text-center text-slate-400">No workflow files uploaded yet.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setInterval(() => {
                fetch(pollEndpoint)
                    ...
                    .then(data => {
                        // Update Text Metrics
                        document.getElementById('stat-total_leads').innerText = data.pipeline
                            .total_leads;
                        ...
                        document.getElementById('stat-book_to_show').innerText = data.conversion_rates
                            .book_to_show_rate !== null ? data.conversion_rates.book_to_show_rate +
                            '%' : '-';
                        document.getElementById('stat-show_to_close').innerText = data.conversion_rates
                            .show_to_close_rate !== null ? data.conversion_rates.show_to_close_rate +
                            '%' : '-';
                        document.getElementById('stat-overall_close').innerText = data.conversion_rates
                            .overall_close_rate !== null ? data.conversion_rates.overall_close_rate +
                            '%' : '-';
                        document.getElementById('stat-avg_deal_volume').innerText = '$' + data
                            .conversion_rates.avg_closed_volume.toLocaleString(undefined, {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2
                            });
                        document.getElementById('stat-total_dials').innerText = data.conversion_rates
                            .total_dials;
                        document.getElementById('stat-total_closes').innerText = data.conversion_rates
                            .total_closes;
