...
<div class="grid md:grid-cols-2 gap-6 mb-8">
    <form action="{{ route($routePrefix.'deliverability.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border p-6 space-y-4">
        @csrf
    ...
    <div class="bg-white rounded-xl shadow-sm border p-6">
        ...
        <p class="text-sm text-slate-600 mb-4">Create a unique test address, send your campaign email to it, and IMAP polling will analyze authentication headers and spam content.</p>
        ...
        @elseif(!$inboundImapConfigured)
            <p class="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                Set <code>EMAIL_CHECKER_IMAP_*</code> variables so inbound emails are analyzed automatically (polled every 5 minutes).
            </p>
        @endif
        <form action="{{ route($routePrefix.'deliverability.inbox') }}" method="POST">
            @csrf
            <button type="submit" class="bg-slate-700 text-white px-4 py-2 rounded-lg" @disabled(!$inboundDomainConfigured)>Generate Test Inbox</button>
        </form>
        @if($inboxes->count())
            <div class="mt-4 space-y-2" id="inbox-list">
                @foreach($inboxes as $inbox)
                    <div class="text-xs font-mono bg-slate-50 p-2 rounded flex justify-between gap-2 inbox-row" data-inbox-id="{{ $inbox->id }}" data-status-url="{{ route($routePrefix.'deliverability.inbox.status', $inbox) }}">
                        <span>{{ $inbox->email_address }}</span>
                        <span class="inbox-status">{{ $inbox->status }}@if($inbox->overall_score) — {{ $inbox->overall_score }}/10 @endif</span>
                    </div>
                @endforeach
            </div>
        @endif
    ...
    <table>
        ...
        <tbody>
            @forelse($tests as $test)
                <tr>
                    <td><a href="{{ route($routePrefix.'deliverability.show', $test) }}" class="text-indigo-600">{{ $test->domain }}</a></td>
...
(function () {
    ...
    const poll = () => {
        document.querySelectorAll('.inbox-row').forEach(async (row) => {
            ...
            if (!statusEl || statusEl.textContent.startsWith('analyzed') || statusEl.textContent.startsWith('expired')) {
                return;
            }

            try {
                const response = await fetch(row.dataset.statusUrl, { headers: { Accept: 'application/json' }, credentials: 'same-origin' });
