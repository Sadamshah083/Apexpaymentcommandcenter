...
@section('content')
    <div class="mb-8 flex items-center justify-between">
        ...
        <div
            class="flex items-center gap-2 bg-green-50 text-green-700 border border-green-200 px-3 py-1.5 rounded-full text-xs font-semibold">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex flex-col justify-between">
            <div>
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span
                        class="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full font-medium">{{ $cpu['cores'] }}
                        Cores</span>
                </div>
    ...
    <div class="grid md:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="space-y-3.5 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <span
                        class="inline-flex items-center gap-1 font-semibold {{ str_starts_with($dbStatus, 'Connected') ? 'text-green-600' : 'text-red-500' }}">
                        <span
                            class="h-2 w-2 rounded-full {{ str_starts_with($dbStatus, 'Connected') ? 'bg-green-500' : 'bg-red-500' }}"></span>
                        {{ $dbStatus }}
                    </span>
                </div>
                <div class="flex justify-between">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex flex-col justify-between">
            ...
            <div class="text-xs text-slate-400 bg-slate-50 border rounded-lg p-3">
                ...
                <p>Queue workers check for jobs in the database. Active processes are managed by systemd supervisor daemon.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 mb-4">Why pushing to the server may take a significant amount of time, and
                    how we optimized it.</p>
                ...
                <div class="space-y-4 text-sm text-slate-600">
                    <div class="bg-slate-50 border border-slate-100 p-4 rounded-lg">
                        ...
                        <p>The root folder contained a huge ZIP archive (<code
                                class="bg-slate-200 text-slate-700 px-1 rounded text-xs">php83.zip</code>, ~34 MB) that
                            wasn't excluded from deployment. As a result, the deploy script compressed this zip into the
                            release tarball, transferring it over SSH on every push. We have modified <code
                                class="text-indigo-600">deploy/run_deploy.py</code> to automatically ignore all <code
                                class="text-indigo-600">.zip</code> files, slashing the upload size by 98%.</p>
                    </div>
                    ...
                    <div class="bg-slate-50 border border-slate-100 p-4 rounded-lg">
                        ...
                        <p>The remote installation script (<code
                                class="bg-slate-200 text-slate-700 px-1 rounded text-xs">deploy/install-app.sh</code>) runs
                            <code class="text-indigo-600">composer install</code>, <code class="text-indigo-600">npm
                                ci</code>, and <code class="text-indigo-600">npm run build</code> directly on the Ubuntu
                            server. On basic hosting plan servers with shared/low CPU cores, compiling assets and resolving
                            packages can take anywhere from 1 to 5 minutes and cause heavy memory spikes.
                        </p>
                    </div>

                    <div class="bg-slate-50 border border-slate-100 p-4 rounded-lg">
                        <p class="font-semibold text-slate-800 mb-1">3. Future Optimization Recommendation</p>
                        <p>To reduce remote build times further, you can compile frontend assets locally or on a CI/CD
                            runner (e.g. GitHub Actions), upload the compiled <code
                                class="bg-slate-200 text-slate-700 px-1 rounded text-xs">public/build</code> folder, and
                            skip the remote <code class="text-indigo-600">npm ci && npm run build</code> stages entirely.
                        </p>
                    </div>
                </div>
            </div>
