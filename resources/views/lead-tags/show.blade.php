                ...
                <div class="app-field sm:col-span-2">
                    ...
                    <div class="flex flex-wrap gap-2 mt-2">
                        @foreach($lists as $list)
                            <label class="app-member-chip">
                                <input type="checkbox" name="list_ids[]" value="{{ $list->id }}" @checked(in_array($list->id, $listIds, true))>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            @if(($counts['imported'] ?? 0) > 0 || ($counts['failed'] ?? 0) > 0)
                <div class="app-card app-card-padded space-y-3 border-indigo-200">
                    ...
                    <p class="app-section-desc">Queue AI enrichment for {{ number_format(($counts['imported'] ?? 0) + ($counts['failed'] ?? 0)) }} imported/failed leads across all matching files.</p>
                    @if($enrichmentConfigured ?? false)
                        <form method="POST" action="{{ route('admin.lead-tags.enrich') }}">
                            ...
                            @endforeach
                            <button type="submit" class="app-btn app-btn-primary app-btn-sm">Enrich matching leads</button>
                        </form>
                    @else
            ...
            @if(($counts['ready_to_distribute'] ?? 0) > 0)
                <div class="app-card app-card-padded space-y-3 border-emerald-200 bg-emerald-50/40">
                    ...
                    <p class="app-section-desc">Distribute {{ number_format($counts['ready_to_distribute']) }} enriched leads from any import.</p>
                    <form method="POST" action="{{ route('admin.lead-tags.distribute') }}" class="space-y-3">
                        ...
                        <button type="submit" class="app-btn app-btn-primary app-btn-sm">Distribute matching leads</button>
                    </form>
                </div>
            @endif
        </div>
            ...
            @if($selectedTagIds !== [] && ($tags ?? collect())->isNotEmpty())
                <details class="app-details">
                    ...
                    <form method="POST" action="{{ route('admin.lead-tags.apply') }}" class="mt-3 space-y-3">
                        ...
                        <button type="submit" class="app-btn app-btn-secondary app-btn-sm">Apply tag to all filtered leads</button>
                    </form>
                </details>
            @endif
        </div>
                        ...
                        <tr>
                            <td>
                                ...
                                @if($lead->city || $lead->state)
                                    <div class="text-xs text-zinc-400">{{ $lead->city }}{{ $lead->city && $lead->state ? ', ' : '' }}{{ $lead->state }}</div>
                                @endif
                            </td>
                            <td class="text-sm">
                                @if($lead->workflow)
                                    <a href="{{ route('admin.workflows.show', $lead->workflow_id) }}" class="app-link">{{ $lead->workflow->name }}</a>
                                    @if($lead->leadList)
                            ...
                            <td>
                                <div class="flex flex-wrap gap-1">
                                    @foreach($lead->tags as $tag)
                                        <a href="{{ route('admin.lead-tags.show', ['tag_ids' => [$tag->id]]) }}" class="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-zinc-100 text-zinc-600 hover:bg-indigo-50" style="border-left: 2px solid {{ $tag->color }}">{{ $tag->name }}</a>
                                    @endforeach
                                </div>
                            </td>
