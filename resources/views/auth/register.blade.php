    ...
    <div class="w-full max-w-md space-y-6">
        ...
        <div class="glass-card rounded-3xl p-8 md:p-10 transition-all duration-300">
            <div class="space-y-6">
                ...
                <form method="POST"
                      action="{{ route('admin.register-workspace') }}"
                      class="space-y-4"
                      data-form-loading
                      data-loading-title="Creating workspace"
                      data-loading-message="Setting up your workspace and admin account…"
                      data-loading-button-text="Creating…">
                    @csrf
                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-warmgrey-500">workspace name</label>
                        <input type="text" name="workspace_name" required placeholder="e.g. Acme Marketing" value="{{ old('workspace_name') }}" class="w-full px-4 py-3 rounded-xl input-glass text-sm">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-warmgrey-500">admin username</label>
                        <input type="text" name="admin_username" required placeholder="e.g. john_doe" value="{{ old('admin_username') }}" class="w-full px-4 py-3 rounded-xl input-glass text-sm">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-warmgrey-500">admin password</label>
                        <input type="password" name="admin_password" required placeholder="••••••••" class="w-full px-4 py-3 rounded-xl input-glass text-sm">
                    </div>
                ...
                <div class="pt-4 border-t border-warmgrey-100 text-center">
                    <p class="text-xs text-warmgrey-500">
                        ...
                        <a href="{{ route('admin.login') }}" class="text-warmgrey-900 hover:underline font-semibold transition-colors">Login here</a>
                    </p>
                </div>
            </div>
        </div>
