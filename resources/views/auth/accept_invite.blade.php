...
<div class="max-w-md mx-auto mt-12 bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
    ...
    <form method="POST" action="{{ route('portal.invite.store', $invitation->token) }}" class="mt-6 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="username" value="{{ old('username', $invitation->username) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm">
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Password</label>
            <input type="password" name="password" required class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm">
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Confirm Password</label>
            <input type="password" name="password_confirmation" required class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm">
        </div>
        <button type="submit" class="w-full py-3 bg-warmgrey-900 hover:bg-warmgrey-800 text-white font-bold rounded-xl">
            Activate Account
        </button>
    </form>
</div>
