                       ...
                       class="ghl-contact-row {{ ($selectedSessionId ?? '') === $session['session_id'] ? 'ghl-contact-row-active' : '' }}">
                        ...
                        <span class="ghl-contact-side">
                            @if($session['last_access_time'])
                                <span class="ghl-contact-time">{{ \Carbon\Carbon::parse($session['last_access_time'])->diffForHumans(short: true) }}</span>
                            @endif
            ...
            @if($selectedSession)
                <div class="ghl-detail-header">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-sm text-zinc-500 mt-0.5">{{ $selectedSession['session_type'] ?? 'user' }} session</p>
                    </div>
                ...
                <section class="ghl-card ghl-conversation">
                    ...
                    <div class="ghl-thread">
                        @forelse($messages as $message)
                            @php
                                $isOutbound = in_array($message['direction'] ?? '', ['outbound', 'out'], true)
                                    || ($message['delivery_status'] ?? '') === 'sent';
                            @endphp
                            <article class="ghl-message {{ $isOutbound ? 'ghl-message-out' : 'ghl-message-in' }}">
                                <div class="ghl-message-bubble">
                                    <div class="ghl-message-body whitespace-pre-wrap">{{ $message['message'] ?: '(attachment or empty message)' }}</div>
                                        ...
                                        <div class="ghl-message-actions">
                                            @foreach($message['attachments'] as $attachment)
                                                @if(!empty($attachment['download_url']))
                                                    <a href="{{ $attachment['download_url'] }}" class="comm-hub-link" target="_blank" rel="noopener">
                ...
                @if(!empty($phoneUsers))
                    <section class="ghl-card mt-4">
                        ...
                        <form method="POST" action="{{ route($routePrefix.'communications.zoom.sms.send') }}" class="space-y-3">
                            @csrf
                            ...
                            <div>
                                ...
                                <input type="hidden" name="sender_phone" id="sms-sender-phone" value="{{ old('sender_phone', $selectedSession['owner_phone'] ?? '') }}">
                            </div>
                            <div>
                                <label class="comm-hub-label block mb-1" for="sms-to">To</label>
                                <input type="tel" id="sms-to" name="to_phone" class="comm-hub-input w-full" required
                                       value="{{ old('to_phone', $selectedSession['other_phone'] ?? '') }}"
