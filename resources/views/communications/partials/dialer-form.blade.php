...
<div class="ghl-dialer-outbound-help comm-hub-card p-4 mb-4 text-sm text-slate-600">
    ...
    <p class="mb-2">Calls are placed from your <strong>Morpheus extension</strong>. Your SIP softphone (Zoiper,
        Linphone, Morpheus web phone) must be registered and online.</p>
    <p class="text-xs text-slate-500">The hub tries the Morpheus API when <code>MORPHEUS_DIAL_METHOD=api</code>,
        otherwise opens your softphone via a SIP link.</p>
    @if ($portalUrl !== '#')
...
<form method="POST" action="{{ route($routePrefix . 'communications.morpheus.calls.originate') }}"
    ...
    <input type="tel" id="{{ $numberInputId }}" name="destination"
        ...
        value="{{ $prefillNumber ?? '' }}" required autocomplete="tel">
    ...
    <div class="ghl-dialer-actions">
        ...
        @endif
        <button type="submit" id="{{ $dialBtnId }}" class="comm-hub-btn ghl-dialer-call-btn">Call with Morpheus
            CX</button>
    </div>
</form>
