                        ...
                        <a href="{{ route(
                            ...
                            <span class="ghl-contact-side">
                                @if ($channel['last_message_sent_time'] ?? null)
                                    <span
                                        class="ghl-contact-time">{{ \Carbon\Carbon::parse($channel['last_message_sent_time'])->diffForHumans(short: true) }}</span>
                                @endif
            ...
            <main class="ghl-detail-panel">
                @if ($selectedThread)
                    ...
                    <section class="ghl-card ghl-conversation">
                        ...
                        <div class="ghl-thread">
                                ...
                                <article class="ghl-message {{ $isOutbound ? 'ghl-message-out' : 'ghl-message-in' }}">
                                    <div class="ghl-message-bubble">
                                        @if (!empty($message['sender']))
                                            <div class="text-xs font-semibold text-zinc-600 mb-1">{{ $message['sender'] }}
                                            </div>
                                        @endif
                                        <div class="ghl-message-body whitespace-pre-wrap">
                                            {{ $message['message'] ?: '(empty message)' }}</div>
                            ...
                            @empty
                                <p class="ghl-empty py-8">No messages in this channel yet, or messages are outside the
                                    available history window.</p>
                            @endforelse
                        </div>
                ...
                @else
                    <div class="ghl-detail-empty">
                        ...
                        <p class="app-page-subtitle max-w-sm">Choose a team chat channel to read the conversation history.
                        </p>
                    </div>
                @endif
            </main>
