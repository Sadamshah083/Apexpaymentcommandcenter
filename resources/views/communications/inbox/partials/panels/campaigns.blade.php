        ...
        @forelse($items as $campaign)
            <div class="flex items-center justify-between p-3 rounded-lg border border-slate-100">
                <div>
                    ...
                    <div class="text-xs text-slate-500">{{ $campaign['dial_mode'] ?? '—' }} ·
                        {{ $campaign['status'] ?? '—' }}</div>
                </div>
                <div class="flex items-center gap-2">
                    <form method="POST"
                        action="{{ route($routePrefix . 'communications.morpheus.campaigns.update', ['id' => $campaign['id']]) }}"
                        class="flex gap-1">
                        @csrf @method('PATCH')
                        <select name="status" class="comm-hub-input text-xs py-1">
                            @foreach (['draft', 'active', 'paused', 'completed', 'archived'] as $status)
                                <option value="{{ $status }}" @selected(($campaign['status'] ?? '') === $status)>
                    ...
                    <form method="POST"
                        action="{{ route($routePrefix . 'communications.morpheus.campaigns.destroy', ['id' => $campaign['id']]) }}"
                        onsubmit="return confirm('Delete campaign?')">@csrf @method('DELETE')<button type="submit"
