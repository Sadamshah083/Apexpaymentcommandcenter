@php $items = $morpheusLeads ?? []; $lists = $morpheusLists ?? []; @endphp

<div class="ghl-card mb-6">
    <h3 class="ghl-card-title">Create dialer lead</h3>
    <form method="POST" action="{{ route($routePrefix.'communications.morpheus.leads.store') }}" class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
        @csrf
...
<div class="ghl-card">
    ...
    <x-data-table>
        <table class="mt-3">
            <thead><tr><th>Name</th><th>Phone</th><th>Status</th><th>Disposition</th><th></th></tr></thead>
            <tbody>
                @forelse($items as $lead)
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route($routePrefix.'communications.morpheus.leads.destroy', ['id' => $lead['id']]) }}" onsubmit="return confirm('Delete lead?')">@csrf @method('DELETE')<button type="submit" class="comm-hub-link text-xs text-red-600">Delete</button></form>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="ghl-empty py-6">No leads found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </x-data-table>
