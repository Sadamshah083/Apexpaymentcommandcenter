<div class="ghl-conv-header">
    ...
    <div class="ghl-conv-header-info">
        ...
        <p class="ghl-conv-header-sub">{{ $contact['phone'] ?? $contact['email'] ?? 'No phone or email' }} · {{ $contact['tag'] ?? 'contact' }}</p>
    </div>
...
<div class="ghl-contact-strip">
    <div class="ghl-contact-strip-item"><span class="ghl-contact-strip-value">{{ $contactStats['activity_count'] ?? 0 }}</span><span class="ghl-contact-strip-label">Activities</span></div>
    <div class="ghl-contact-strip-item"><span class="ghl-contact-strip-value">{{ $contactStats['call_count'] ?? 0 }}</span><span class="ghl-contact-strip-label">Calls</span></div>
    <div class="ghl-contact-strip-item"><span class="ghl-contact-strip-value">{{ $contactStats['sms_count'] ?? 0 }}</span><span class="ghl-contact-strip-label">SMS</span></div>
    <div class="ghl-contact-strip-item"><span class="ghl-contact-strip-value">{{ $contactStats['voicemail_count'] ?? 0 }}</span><span class="ghl-contact-strip-label">Voicemail</span></div>
</div>
        ...
        <div class="ghl-timeline">
            @forelse($timeline as $event)
                ...
                <article class="ghl-timeline-item {{ $isOut ? 'ghl-timeline-item-out' : '' }}">
                    <span class="ghl-timeline-icon ghl-timeline-icon-{{ $eventType }}">{{ strtoupper(substr($eventType, 0, 1)) }}</span>
                    <div class="ghl-timeline-bubble">
                        ...
                        @elseif($eventType === 'voicemail')
                            ...
                            @if(!empty($event['transcription']))
                                <div class="ghl-timeline-body" style="font-style: italic;">{{ $event['transcription'] }}</div>
                            @endif
                        @endif
                        <div class="ghl-timeline-meta">{{ $event['detail'] }} · {{ \Carbon\Carbon::parse($event['at'])->format('M j, g:i A') }}</div>
                        <div class="ghl-timeline-actions">
                            ...
                            @elseif($eventType === 'sms' && !empty($event['session_id']))
                                <a href="{{ route($routePrefix.'communications.index', ['channel' => 'sms', 'session' => $event['session_id']]) }}" class="comm-hub-link">Open SMS</a>
