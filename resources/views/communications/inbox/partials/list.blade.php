    ...
    <div class="ghl-inbox-list-scroll">
        @forelse($sidebarItems as $item)
            <a href="{{ $item['url'] }}"
               class="ghl-inbox-row {{ !empty($item['active']) ? 'ghl-inbox-row-active' : '' }}">
                <span class="ghl-inbox-row-avatar ghl-inbox-row-avatar-{{ $item['kind'] ?? 'contact' }}">{{ $item['avatar'] }}</span>
                ...
                <span class="ghl-inbox-row-meta">
                    @if(!empty($item['time']))
                        <span class="ghl-inbox-row-time">{{ \Carbon\Carbon::parse($item['time'])->diffForHumans(short: true) }}</span>
                    @endif
                    @if(!empty($item['badge']))
        ...
        @empty
            <div class="ghl-inbox-empty" style="min-height: 12rem; padding: 2rem 1rem;">
                ...
                @endif
                @if(($channel ?? '') === 'calls' || ($channel ?? '') === 'recordings')
                    <p class="text-xs text-zinc-500 mt-2">Widen the date range in the toolbar if you expect older calls.</p>
                @endif
            </div>
        @endforelse
    </div>
