@switch($panel)
    @case('contact')
        <div class="ghl-inbox-conversation-scroll">
            ...
            @else
                @include('communications.inbox.partials.empty', ['title' => 'Contact not found', 'message' => 'Try widening the date range or pick another contact from the list.'])
            @endif
        </div>
        @break
    ...
    @default
        <div class="ghl-inbox-empty">
            ...
            <p>Choose someone from the {{ strtolower($channels[$channel]['label'] ?? 'inbox') }} list, or use the tools on the right to place a call.</p>
            <div class="ghl-inbox-empty-actions">
                <a href="{{ route($routePrefix.'communications.index', ['channel' => 'inbox']) }}" class="comm-hub-btn comm-hub-btn-secondary comm-hub-btn-sm">Open inbox</a>
                <a href="{{ route($routePrefix.'communications.index', array_merge($baseQuery, ['panel' => 'dialer'])) }}" class="comm-hub-btn comm-hub-btn-sm">Open dialer</a>
                @if($channel === 'sms')
                    <a href="{{ route($routePrefix.'communications.index', array_merge($baseQuery, ['panel' => 'compose_sms'])) }}" class="comm-hub-btn comm-hub-btn-sm">New SMS</a>
                @endif
            </div>
        </div>
@endswitch
