@if (!empty($connectionDiagnostics['messages']) || !empty($warnings))
    <div class="comm-hub-alert comm-hub-alert-warning mb-4 text-left">
        <p class="font-semibold text-sm mb-2">Morpheus CX is connected, but call data is not available yet.</p>
        ...
        <p class="text-xs mt-3 opacity-90">
            Open <a
                href="{{ route($routePrefix . 'communications.index', ['channel' => 'inbox', 'panel' => 'settings']) }}"
                class="underline font-semibold">Settings → Refresh data</a>
            after updating your Morpheus CX settings.
        </p>
    </div>
@endif
