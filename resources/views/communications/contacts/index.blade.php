        ...
        <aside class="ghl-list-panel">
            <div class="ghl-list-toolbar">
                <form method="GET" action="{{ route($routePrefix.'communications.index') }}" class="ghl-search-form">
                    ...
                    @endif
                    <input type="search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search name, phone, email" class="ghl-search-input">
                </form>
                       ...
                       class="ghl-contact-row {{ ($selectedKey ?? '') === $contact['contact_key'] ? 'ghl-contact-row-active' : '' }}">
                        ...
                        <span class="ghl-contact-side">
                            @if($contact['last_activity_at'])
                                <span class="ghl-contact-time">{{ \Carbon\Carbon::parse($contact['last_activity_at'])->diffForHumans(short: true) }}</span>
                            @endif
                            <span class="ghl-tag">{{ $contact['tag'] ?? 'contact' }}</span>
                        </span>
                    </a>
                @empty
                    <div class="ghl-empty">No contacts in the last {{ config('integrations.communications.default_days', 14) }} days.</div>
                @endforelse
            </div>
        </aside>
        ...
        <main class="ghl-detail-panel">
            ...
            @else
                <div class="ghl-detail-empty">
                    ...
                    <p class="app-page-subtitle max-w-sm">Choose someone from the list to see their profile and call history.</p>
                </div>
            @endif
        </main>
    </div>
