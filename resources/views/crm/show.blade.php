...
@section('content')
    <div class="mb-6">
        ...
        <div class="flex flex-wrap justify-between items-start gap-4 mt-2">
            ...
            <div class="flex flex-wrap gap-2">
                <form action="{{ route('admin.crm.reupload', $crm) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <label class="text-sm border px-3 py-2 rounded-lg hover:bg-slate-50 cursor-pointer">
                        ...
                        <input type="file" name="file" accept=".csv,.txt" class="hidden"
                            onchange="if(confirm('Re-import CSV? Completed research is kept unless business name or address changed.')) this.form.submit()">
                    </label>
                </form>
                <a href="{{ route('admin.crm.export', $crm) }}"
                    class="text-sm border px-3 py-2 rounded-lg hover:bg-slate-50">Export CSV</a>
                @if ($crm->failed_count > 0 || $crm->processing_count > 0)
                    <form action="{{ route('admin.crm.retry-failed', $crm) }}" method="POST"
                        onsubmit="return confirm('Re-queue all failed/stuck leads?')">
                        @csrf
                        <button type="submit" class="text-sm bg-indigo-600 text-white px-3 py-2 rounded-lg">Re-run Failed
                            ({{ $crm->failed_count + $crm->processing_count }})</button>
                    </form>
                @endif
                <form action="{{ route('admin.crm.destroy', $crm) }}" method="POST"
                    onsubmit="return confirm('Delete this campaign and all leads?')">
                    @csrf @method('DELETE')
                    <button type="submit"
                        class="text-sm text-red-600 border border-red-200 px-3 py-2 rounded-lg">Delete</button>
                </form>
            </div>
        </div>
    </div>
    ...
    @if ($crm->status === 'pending' && $crm->total_leads === 0)
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 text-blue-800 text-sm">Waiting to import CSV…
        </div>
    @endif
        ...
        <div id="progress-panel" class="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-6">
            ...
            <div class="w-full bg-amber-200 rounded-full h-3">
                <div id="progress-bar" class="bg-indigo-600 h-3 rounded-full progress-bar-live"
                    style="width: {{ $crm->progressPercent() }}%"></div>
            </div>
            <p class="text-xs text-amber-700 mt-2">
                <span id="progress-stats">{{ $crm->completed_count }} done · {{ $crm->processing_count }} processing ·
                    {{ $crm->pending_count }} pending · {{ $crm->failed_count }} failed</span>
            </p>
            <p class="text-xs text-amber-600 mt-2">Faster: run <code class="bg-white/70 px-1 rounded">php artisan queue:work
                    database --sleep=1</code> in 3–4 terminals.</p>
        </div>
    @endif
                ...
                @forelse($leads as $lead)
                    <tr>
                        ...
                        <td>
                            ...
                            @if ($lead->isEnriched())
                                <span
                                    class="ml-1 px-2 py-0.5 rounded text-xs bg-emerald-100 text-emerald-800">enriched</span>
                            @endif
                        </td>
                        <td>
                            <a href="{{ route('admin.crm.leads.show', [$crm, $lead]) }}"
                                class="text-indigo-600 hover:underline text-xs">View</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="9" class="text-center py-8 text-slate-500">No leads match your filters.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
            ...
            (function() {
                ...
                start('{{ route('admin.crm.progress', $crm) }}', (data) => {
                    ...
                    document.getElementById('progress-stats').textContent =
                        data.completed + ' done · ' + data.processing + ' processing · ' + data.pending +
                        ' pending · ' + data.failed + ' failed';
