    ...
    <form action="{{ route('crm.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required
                placeholder="Houston Plumbing Leads — March 2026"
        ...
        <div>
            ...
            <input type="file" name="file" accept=".csv,.txt" required
                class="w-full border rounded-lg px-3 py-2">
            <p class="text-xs text-slate-500 mt-1">Max 50MB. First row must be headers. Any extra columns are preserved.</p>
        </div>
        ...
        <div class="bg-slate-50 rounded-lg p-4 text-xs text-slate-600 space-y-2">
            ...
            <ul class="list-disc list-inside space-y-1">
                <li>Auto-maps <strong>Company Name</strong>, <strong>Business Name</strong>, full or split address columns</li>
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-900">
            <strong>Speed tip:</strong> Open 3–4 terminal windows and run this in each for parallel research (~3–4× faster):
