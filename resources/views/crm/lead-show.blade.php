    ...
    <div class="grid lg:grid-cols-2 gap-6 mb-6">
        <div class="bg-white rounded-xl border p-5">
            ...
            <dl class="text-sm space-y-2">
                ...
                <div class="flex justify-between border-b pb-2">
                    <dt class="text-slate-500">Input Phone</dt>
                    <dd>{{ $lead->input_phone ?? '—' }}</dd>
                </div>
                <div class="flex justify-between border-b pb-2">
                    <dt class="text-slate-500">Input Email</dt>
                    <dd>{{ $lead->input_email ?? '—' }}</dd>
                </div>
            </dl>
        ...
        <div class="bg-white rounded-xl border p-5">
            ...
            <dl class="text-sm space-y-2">
                <div class="flex justify-between border-b pb-2">
                    <dt class="text-slate-500">Verified Address</dt>
                    <dd class="text-right max-w-[60%]">{{ $lead->physical_address ?? '—' }}</dd>
                </div>
                <div class="flex justify-between border-b pb-2">
                    <dt class="text-slate-500">Primary Service</dt>
                    <dd>{{ $lead->primary_service ?? '—' }}</dd>
                </div>
                <div class="flex justify-between border-b pb-2">
                    <dt class="text-slate-500">Operating Hours</dt>
                    <dd class="text-right max-w-[60%]">{{ Str::limit($lead->operating_hours ?? '—', 80) }}</dd>
                </div>
                <div class="flex justify-between border-b pb-2">
                    <dt class="text-slate-500">POS / Software</dt>
                    <dd>{{ $lead->field_service_software ?? ($lead->pos_system ?? '—') }}</dd>
                </div>
                <div class="flex justify-between border-b pb-2">
                    <dt class="text-slate-500">Confidence</dt>
                    <dd>{{ ucfirst($lead->confidence ?? 'unknown') }}</dd>
                </div>
                <div class="flex justify-between border-b pb-2">
                    <dt class="text-slate-500">Model</dt>
                    <dd class="text-xs">{{ $lead->model_used ?? '—' }}</dd>
                </div>
            </dl>
        </div>
    </div>
                    ...
                    <li>
                        @if (!empty($source['url']))
                            <a href="{{ $source['url'] }}" target="_blank" rel="noopener"
                                class="text-indigo-600 hover:underline">{{ $source['title'] ?? $source['url'] }}</a>
                        @else
                            {{ $source['title'] ?? 'Source' }}
                        @endif
                    </li>
