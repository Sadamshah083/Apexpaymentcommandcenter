...
<div class="app-page space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div>
            <h1 class="app-page-title">Closer Team</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach($teamMetrics as $metric)
            ...
            @endphp
            <a href="{{ route('portal.closer-team.dashboard', array_filter(['closer' => $metric['user']->id, 'search' => request('search')])) }}"
    ...
    <div class="app-card app-card-padded">
        <form method="GET" action="{{ route('portal.closer-team.dashboard') }}" class="flex flex-wrap items-end gap-3 mb-4">
            <div class="app-field flex-1 min-w-[200px]">
                <label for="search" class="app-label">Search leads</label>
                <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Search business, owner, or email..." class="app-input">
            </div>
            ...
            @if(request()->filled('search') || request()->filled('closer'))
