                ...
                @foreach (['dials' => 'Dials', 'conversations' => 'Conversations', 'decision_maker_contacts' => 'DM Contacts', 'discoveries' => 'Discoveries'] as $key => $label)
                    ...
                    <div class="app-card app-card-padded">
                        ...
                        <p class="app-kpi-value">
                            <span id="workspace-sync-metric-{{ $key }}-actual">{{ $m['actual'] }}</span><span
                                class="text-base font-semibold text-zinc-400"> / <span
                                    id="workspace-sync-metric-{{ $key }}-target">{{ $m['target'] }}</span></span>
                        </p>
            ...
            <div class="{{ request()->is('admin*') ? 'lg:col-span-2' : '' }} space-y-6">
                <div class="app-card app-card-padded">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                        <div>
                            <h2 class="app-section-title">{{ request()->is('admin*') ? 'Active leads' : 'Assigned leads' }}
                            </h2>
                            <p class="app-section-desc">
                                {{ request()->is('admin*') ? 'Leads released to your workspace.' : 'Update stage and log outreach.' }}
                            </p>
                        </div>

                        <!-- Search & Filter Form -->
                        <form method="GET"
                            action="{{ request()->is('admin*') ? route('admin.workflows.index') : route('portal.dashboard') }}"
                            ...
                            <div class="app-search-wrap">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    placeholder="Search business, owner…" class="app-input">
                            </div>
                            <select name="phase" onchange="this.form.submit()" class="app-input !w-auto">
                                <option value="">All phases</option>
                                @foreach ($pipelinePhases ?? [] as $value => $label)
                                    <option value="{{ $value }}"
                                        {{ request('phase') === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if (request()->anyFilled(['search', 'phase']))
                                <a href="{{ request()->is('admin*') ? route('admin.workflows.index') : route('portal.dashboard') }}"
                                    class="app-btn app-btn-secondary app-btn-sm" title="Clear filters">Clear</a>
                            @endif
                        </form>
                    </div>
                        ...
                        <x-data-table :paginator="$leads">
                            <table>
                                ...
                                <tbody id="workspace-sync-leads-body"
                                    data-sync-mode="{{ request()->anyFilled(['search', 'stage', 'tier']) || $leads->currentPage() > 1 ? 'patch' : 'replace' }}">
                                        ...
                                        <tr data-lead-id="{{ $lead->id }}">
                                            ...
                                            <td>
                                                ...
                                                @if ($lead->direct_phone && $lead->direct_phone !== 'Not Publicly Available')
                                                    <div class="text-xs text-zinc-400 mt-0.5">{{ $lead->direct_phone }}
                                                    </div>
                                                @endif
                                            ...
                                            <td>
                                                <span
                                                    class="app-badge app-badge-info">{{ $lead->payment_processor ?: 'Unknown' }}</span>
                                            </td>
                                            <td>
                                                <span
                                                    class="app-badge app-badge-muted">{{ \App\Support\SalesOps::pipelinePhaseLabel($lead->pipeline_phase) }}</span>
                                                @if ($lead->setter_status)
                                                    <div class="text-[10px] text-zinc-400 mt-1">
                                                        {{ \App\Support\SalesOps::setterStatusLabel($lead->setter_status) }}
                                                    </div>
                                                @endif
                                            </td>
                                            <td class="text-right">
                                                <a href="{{ route('portal.leads.show', $lead->id) }}" class="app-icon-btn"
                                                    title="Open lead">
                                                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                                        aria-hidden="true">
                    ...
                    <div class="app-card app-card-padded">
                        ...
                        @if ($workflows->isEmpty())
                            ...
                            <a href="{{ route('admin.workflows.create') }}"
                                class="app-link text-sm inline-block mt-3">Import your first list</a>
                        @else
                            <div id="workspace-sync-workflows" class="space-y-3"
                                data-sync-mode="{{ $workflows->currentPage() > 1 ? 'patch' : 'replace' }}">
                                    ...
                                    <div class="app-import-card" data-workflow-id="{{ $wf->id }}">
                                        ...
                                        @if ($wf->leadList || !empty($wf->import_tag_ids))
                                            <div class="mt-2 text-xs text-zinc-500">
                                                @if ($wf->leadList)
                                                    <span>List: <strong
                                                            class="text-zinc-700">{{ $wf->leadList->name }}</strong></span>
                                                @endif
                                                @if (!empty($wf->import_tag_ids))
                                                    <span class="{{ $wf->leadList ? 'ml-2' : '' }}">
                                                        <a href="{{ route('admin.lead-tags.show', ['tag_ids' => $wf->import_tag_ids]) }}"
                                                            class="app-link">Tagged import</a>
                                                    </span>
                                                @endif
                                            </div>
                                        @endif
                                        <div class="mt-3 flex items-center justify-end gap-3">
                                            ...
                                            @if (in_array($wf->status, ['pending', 'extracting']))
                                                <form method="POST"
                                                    action="{{ route('admin.workflows.pause', $wf->id) }}"
                                                    class="inline">
                                                    @csrf
                                                    <button type="submit"
                                                        class="app-link text-xs text-amber-700">Pause</button>
                                                </form>
                                            @endif
                                            @if ($wf->status === 'paused')
                                                <form method="POST"
                                                    action="{{ route('admin.workflows.resume', $wf->id) }}"
                                                    class="inline">
                                                    @csrf
                                                    <button type="submit"
                                                        class="app-link text-xs text-emerald-700">Resume</button>
                                                </form>
                                            @endif
                                            <form method="POST" action="{{ route('admin.workflows.destroy', $wf->id) }}"
                                                class="inline"
                                                onsubmit="return confirm('Delete this pipeline and all {{ $wf->total_leads }} lead records from the database?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="app-link text-xs text-rose-600">Delete</button>
                                            </form>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            <x-pagination :paginator="$workflows"
                                class="app-data-table-footer !border-t !border-slate-100 !bg-transparent !px-0" />
                        @endif
                    </div>
                </div>
            @endif
