    ...
    <div class="app-page space-y-6">
        ...
        @if ($lead->isSetterLocked() && $user->isAppointmentSetter($workspace->id))
            <div class="app-alert app-alert-warning">
                <p class="app-alert-title">This lead has been handed off to the closers team and is read-only for setters.
                </p>
            </div>
        @endif
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="app-card app-card-padded space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div><span class="text-zinc-500">Owner</span>
                            <p class="font-semibold">{{ $lead->owner_name ?: '—' }}</p>
                        </div>
                        <div><span class="text-zinc-500">Email</span>
                            <p class="font-semibold">{{ $lead->direct_email ?: $lead->input_email ?: '—' }}</p>
                        </div>
                        <div><span class="text-zinc-500">Phone</span>
                            <p class="font-semibold">{{ $lead->direct_phone ?: $lead->input_phone ?: '—' }}</p>
                        </div>
                        <div><span class="text-zinc-500">Website</span>
                            <p class="font-semibold">{{ $lead->website ?: '—' }}</p>
                        </div>
                        <div><span class="text-zinc-500">Location</span>
                            <p class="font-semibold">
                                {{ trim(($lead->city ?: '') . ', ' . ($lead->state ?: ''), ', ') ?: '—' }}
                            </p>
                        </div>
                        <div><span class="text-zinc-500">Processor</span>
                            <p class="font-semibold">{{ $lead->payment_processor ?: '—' }}</p>
                        </div>
                    </div>
                    ...
                    @if ($canEditSetter)
                        <div class="border-t border-zinc-100 pt-6 space-y-3">
                            ...
                            <p class="text-xs text-zinc-500">Pick a status and add a note about what happened. It appears on
                    ...
                    @if ($canEditCloser && !$lead->isCloserLocked())
                        <form method="POST" action="{{ route('portal.leads.closer-status', $lead->id) }}"
                            class="space-y-4 border-t border-zinc-100 pt-6">
                            @csrf
                                ...
                                @foreach ($closerStatuses as $value => $label)
                                    @if (
                                        $value !== 'sale_made' ||
                                            ...
                                            $user->isSuperAdmin($workspace->id) ||
                                            $user->isAdmin($workspace->id))
                                        <option value="{{ $value }}" @selected($lead->closer_status === $value)>
                                            {{ $label }}</option>
                                    @endif
                                @endforeach
                            </select>
                            <textarea name="notes" rows="3" class="app-input"
                                placeholder="What happened on this call or follow-up? (optional)"></textarea>
