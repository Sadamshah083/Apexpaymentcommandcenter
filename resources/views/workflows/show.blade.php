...
@section('content')
    @php
        ...
        $pct = $workflow->total_leads > 0 ? (int) round(($attempted / $workflow->total_leads) * 100) : 0;
    ...
    <div class="app-page space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
                ...
                <p class="app-page-subtitle">{{ number_format($workflow->total_leads) }} leads &middot;
                    {{ $workflow->original_filename }}</p>
            </div>
                ...
                <div class="app-card app-card-padded space-y-5">
                    <div class="app-step-header">
                        ...
                        <div>
                            ...
                            <p class="app-section-desc">We auto-detected your spreadsheet. Confirm business name, then
                                start.</p>
                        </div>
                    </div>
                    ...
                    @if (!empty($workflow->column_mapping['business_name']))
                        <div class="app-alert app-alert-success">
                            <p class="app-alert-title">Mapped
                                <strong>{{ $workflow->column_mapping['business_name'] }}</strong> to Business Name
                            </p>
                        </div>
                            ...
                            <select name="selected_sheet" id="selected_sheet" class="app-input max-w-xs">
                                @foreach ($workflow->sheets as $sheetName)
                                    <option value="{{ $sheetName }}"
                                        {{ $workflow->selected_sheet === $sheetName ? 'selected' : '' }}>{{ $sheetName }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="app-field">
                            ...
                            <select name="mapping[business_name]" required class="app-input">
                                ...
                                @foreach ($headers as $h)
                                    @if ($h !== '')
                                        <option value="{{ $h }}"
                                            {{ ($workflow->column_mapping['business_name'] ?? '') === $h ? 'selected' : '' }}>
                                            {{ $h }}</option>
                                    @endif
                                @endforeach
                            </select>
                            ...
                            <div class="app-field">
                                ...
                                <select name="mapping[{{ $key }}]" class="app-input">
                                    ...
                                    @foreach ($headers as $h)
                                        @if ($h !== '')
                                            <option value="{{ $h }}"
                                                {{ ($workflow->column_mapping[$key] ?? '') === $h ? 'selected' : '' }}>
                                                {{ $h }}</option>
                                        @endif
                                    @endforeach
                                </select>
                ...
                <div class="app-card app-card-padded space-y-4">
                    <div class="app-step-header">
                        ...
                        <div>
                            ...
                            <p class="app-section-desc">HubSpot-style lists and tags — every imported lead is added to a
                                static list and tagged for filtering.</p>
                        </div>
                    </div>
                    ...
                    <div class="app-field">
                        ...
                        <input type="text" name="tag_names" id="tag_names" class="app-input"
                            placeholder="e.g. cold-outreach, texas, june-2026">
                        <p class="app-field-hint">Comma-separated. Tags trace leads across imports — use them for batch
                            enrich and assign in Lead Tags.</p>
                    </div>
                ...
                <div class="app-card app-card-padded space-y-4 border-zinc-300">
                    <div class="app-step-header">
                        ...
                        <div>
                            ...
                            <p class="app-section-desc">Leads are imported unassigned. Enrichment and setter distribution
                                are separate steps you can run later.</p>
                        </div>
                    </div>

                    <label class="app-checkbox-row">
                        <input type="checkbox" name="run_enrichment_on_import" value="1" @checked($enrichmentConfigured ?? false)
                            @disabled(!($enrichmentConfigured ?? false))>
                        <span class="app-checkbox-row-text">
                            <strong>Run AI enrichment now</strong>
                            @if (!($enrichmentConfigured ?? false))
                                <span class="block text-xs text-zinc-500 font-normal mt-0.5">Requires GEMINI_API_KEY or
                                    OPENROUTER_API_KEY on the server.</span>
                            @else
                                <span class="block text-xs text-zinc-500 font-normal mt-0.5">Uncheck to import only —
                                    enrich from this page when ready.</span>
                            @endif
                        </span>
                    </label>
                    ...
                    <label class="app-checkbox-row">
                        ...
                        <span class="app-checkbox-row-text">I've reviewed the column mapping. Duplicate US phone numbers in
                            this workspace will be skipped.</span>
                    </label>
            ...
            <div class="app-card app-card-padded space-y-5">
                ...
                @if ($workflow->total_leads > 0)
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs font-semibold text-zinc-500">
                            ...
                            <span id="workspace-sync-workflow-progress-label">{{ $pct }}% &middot;
                                {{ $attempted }} / {{ $workflow->total_leads }}</span>
                        </div>
                        <div class="app-progress-track">
                            <div id="workspace-sync-workflow-progress-bar" class="app-progress-fill"
                                style="width: {{ $pct }}%"></div>
                        </div>
                    </div>
                @endif
                ...
                @if (($workflow->discarded_duplicates ?? 0) > 0)
                    <div class="app-alert app-alert-info">
                        <p class="app-alert-title">{{ number_format($workflow->discarded_duplicates) }} duplicate phone
                            numbers skipped</p>
                        <p class="app-alert-desc">Rows with the same US number as an existing lead in this workspace were
                            not imported.</p>
                    </div>
                @endif
                ...
                @if (!empty($workflow->import_tag_ids) || ($leadList ?? null))
                    <div
                        class="app-alert app-alert-info flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            @if ($leadList ?? null)
                                <p class="app-alert-title">List: {{ $leadList->name }}</p>
                            @endif
                            @if (!empty($workflow->import_tag_ids))
                                <p class="app-alert-desc">Batch enrich or assign all leads with these tags — including from
                                    other files.</p>
                        ...
                        @if (!empty($workflow->import_tag_ids))
                            <a href="{{ route('admin.lead-tags.show', ['tag_ids' => $workflow->import_tag_ids]) }}"
                                class="app-btn app-btn-secondary app-btn-sm whitespace-nowrap">Open in Lead Tags</a>
                        @endif
                    </div>
                @endif
                ...
                @if ($importedCount > 0 && !$workflow->isProcessing() && ($enrichmentConfigured ?? false))
                    <div
                        class="app-alert app-alert-warning flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <p class="app-alert-title">{{ number_format($importedCount) }} leads imported — enrichment not
                                started</p>
                            <p class="app-alert-desc">Run AI enrichment when you're ready. Distribution to setters is a
                                separate step after enrichment.</p>
                        </div>
                        @if ($enrichmentConfigured ?? false)
                            <form method="POST" action="{{ route('admin.workflows.enrich', $workflow->id) }}">
                                @csrf
                                <button type="submit" class="app-btn app-btn-primary app-btn-sm whitespace-nowrap">Start
                                    enrichment</button>
                            </form>
                        @endif
                    </div>
                ...
                @if ($readyToDistribute > 0 && !$workflow->isProcessing())
                    <div class="app-card app-card-padded space-y-4 border-emerald-200 bg-emerald-50/40">
                        <div>
                            ...
                            <p class="app-section-desc">{{ number_format($readyToDistribute) }} enriched leads ready ·
                                round-robin to selected setters</p>
                        </div>
                        <form method="POST" action="{{ route('admin.workflows.distribute', $workflow->id) }}"
                            class="space-y-4">
                            @csrf
                            <div class="flex flex-wrap gap-2">
                                @foreach ($team as $member)
                                    <label class="app-member-chip">
                                        ...
                                        <span
                                            class="app-member-chip-role">{{ \App\Support\SalesOps::roleLabel($member->pivot->role) }}</span>
                                    </label>
                                @endforeach
                            </div>
                            <button type="submit" class="app-btn app-btn-primary app-btn-sm">Distribute
                                {{ number_format($readyToDistribute) }} leads</button>
                        </form>
                    </div>
                @endif
                ...
                @if (($workflow->pending_verification_count ?? 0) > 0)
                    <div
                        class="app-alert app-alert-warning flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <p class="app-alert-title"><span
                                    id="workspace-sync-workflow-pending-review">{{ $workflow->pending_verification_count }}</span>
                                leads ready for review</p>
                        ...
                        <form method="POST" action="{{ route('admin.workflows.approve-leads', $workflow->id) }}">
                            ...
                            @endforeach
                            <button type="submit" class="app-btn app-btn-primary app-btn-sm whitespace-nowrap">Approve
                                all</button>
                        </form>
                    </div>
                @endif
                ...
                @if (!($enrichmentConfigured ?? true))
                    <div class="app-alert app-alert-danger">
                        ...
                        <p class="app-alert-desc">
                            {{ $enrichmentConfigMessage ?? 'Add GEMINI_API_KEY or OPENROUTER_API_KEY to the server .env file.' }}
                        </p>
                    </div>
                @elseif(
                    ($enrichmentStatus['gemini']['state'] ?? '') === 'depleted' &&
                        ($enrichmentStatus['openrouter']['state'] ?? '') === 'not_configured')
                    <div class="app-alert app-alert-danger">
                        <p class="app-alert-title">Gemini credits depleted</p>
                        <p class="app-alert-desc">Top up in AI Studio or add OPENROUTER_API_KEY for fallback enrichment.
                        </p>
                    </div>
                @elseif(($enrichmentStatus['gemini']['state'] ?? '') === 'depleted')
                    <div class="app-alert app-alert-warning">
                        <p class="app-alert-title">Gemini credits depleted — using OpenRouter fallback</p>
                        <p class="app-alert-desc">Enrichment will skip Gemini and use OpenRouter until credits are
                            restored.</p>
                    </div>
                @endif
                ...
                @if (($retryableFailedLeads ?? 0) > 0)
                    <div
                        class="app-alert app-alert-warning flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <p class="app-alert-title">{{ number_format($retryableFailedLeads) }} leads failed enrichment
                            </p>
                            <p class="app-alert-desc">Open the Failed tab below for details. After API keys are configured,
                                retry the batch.</p>
                        </div>
                        @if ($enrichmentConfigured ?? false)
                            <form method="POST" action="{{ route('admin.workflows.retry-failed', $workflow->id) }}">
                                @csrf
                                <button type="submit" class="app-btn app-btn-primary app-btn-sm whitespace-nowrap">Retry
                                    failed leads</button>
                            </form>
                        @endif
                    </div>
                ...
                <span id="workspace-sync-workflow-pending-review-2"
                    class="hidden">{{ $workflow->pending_verification_count ?? 0 }}</span>
                <span id="workspace-sync-workflow-assigned"
                    class="hidden">{{ $workflow->assigned_leads_count ?? 0 }}</span>
            </div>
                    ...
                    <x-data-table :paginator="$leads" min-width="640px">
                        <table>
                            ...
                            <tbody id="workspace-sync-pipeline-leads">
                                    ...
                                    <tr data-lead-id="{{ $lead->id }}" data-lead-status="{{ $lead->status }}">
                                        ...
                                        <td>
                                            ...
                                            @if ($lead->status === 'failed' && $lead->error_message)
                                                <div class="text-xs text-rose-600 mt-1 max-w-xs">
                                                    {{ Str::limit($lead->error_message, 120) }}</div>
                                            @endif
                                        </td>
                                        <td class="text-right whitespace-nowrap">
                                            @if ($lead->status === 'pending_verification')
                                                <div class="flex items-center justify-end gap-1">
                                                    <form method="POST"
                                                        action="{{ route('admin.leads.approve', $lead->id) }}">
                                                        @csrf
                                                        <button type="submit"
                                                            class="app-btn app-btn-success app-btn-sm">Approve</button>
                                                    </form>
                                                    <form method="POST"
                                                        action="{{ route('admin.leads.reject', $lead->id) }}">
                                                        @csrf
                                                        <button type="submit"
                                                            class="app-btn app-btn-ghost-danger app-btn-sm">Reject</button>
                                                    </form>
                                                </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-8 text-slate-500">Leads are queued. They
                                            will appear here as enrichment runs.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
...
@push('scripts')
    ...
    <script>
        ...
        document.getElementById('pipeline-lead-filters')?.addEventListener('click', (event) => {
            ...
            document.querySelectorAll('#pipeline-lead-filters button').forEach((b) => b.classList.toggle(
                'is-active', b === btn));
