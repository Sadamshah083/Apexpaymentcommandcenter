#!/usr/bin/env python3
"""Deploy assign-leads modal polish (select all/unselect all, responsive layout, sync fix)."""
...
def main() -> int:
    ...
    ssh = connect()
    print("Uploading assign modal polish...")
    upload_files(ssh, pairs, app_root=REMOTE_APP)
    print("Building assets...")
    print(sudo_run_batch(ssh, [
        f"cd {REMOTE_APP} && npm run build > /tmp/vite-assign-modal.log 2>&1; echo BUILD:$?; tail -n 30 /tmp/vite-assign-modal.log",
        f"chown -R www-data:www-data {REMOTE_APP}/public/build",
        f"cd {REMOTE_APP} && sudo -u www-data php artisan view:clear",
        f"cd {REMOTE_APP} && sudo -u www-data php artisan cache:clear",
    ]))
    ssh.close()
    print("Assign modal polish deployed.")
    return 0
