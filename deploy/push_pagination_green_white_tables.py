#!/usr/bin/env python3
"""Deploy green pagination + white UM table headers."""
...
def main() -> int:
    ...
    ssh = connect()
    print("Uploading green pagination + white table headers...")
    upload_files(ssh, pairs, app_root=REMOTE_APP)
    print("Building assets...")
    print(sudo_run_batch(ssh, [
        f"cd {REMOTE_APP} && npm run build > /tmp/vite-pag-green.log 2>&1; echo BUILD:$?; tail -n 25 /tmp/vite-pag-green.log",
        f"chown -R www-data:www-data {REMOTE_APP}/public/build",
        f"cd {REMOTE_APP} && sudo -u www-data php artisan view:clear",
        f"cd {REMOTE_APP} && sudo -u www-data php artisan cache:clear",
    ]))
    ssh.close()
    print("Green pagination + white headers deployed.")
    return 0
