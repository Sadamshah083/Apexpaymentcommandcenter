#!/usr/bin/env python3
"""Deploy 110% zoom / short-viewport dialer responsiveness."""
...
set -e
cd /var/www/apexone
echo MARKER:$(grep -c '110% zoom' resources/css/comm-hub-ghl-theme.css || true)
...
for k,v in m.items():
    f=str(v.get('file',''))
    if f.endswith('.css') and ('communications-hub' in f or 'app-' in f):
        t=(Path('public/build')/f).read_text(errors='replace')
        print(f, 'zoom110=', '110% zoom' in t or 'short viewport' in t, 'maxh980=', '@media (max-height: 980px)' in t)
PY
