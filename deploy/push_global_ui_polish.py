#!/usr/bin/env python3
"""Global purple headers, gray type, full-width tables, no Rows pagination."""
...
def main() -> int:
    ...
    buf.seek(0)
    remote_tar = "/tmp/apexone-global-ui.tar.gz"
    sftp = ssh.open_sftp()
...
print('purple_header', '#f3e8ff' in css and '#5b21b6' in css)
print('no_rows_dropdown', 'app-pagination-per-page' not in pag and 'Rows' not in pag)
print('full_width', 'max-width: none' in css)
print('tight_main', 'padding: 0.75rem 1rem 1.25rem' in css)
PY
echo DONE
"""
