#!/usr/bin/env python3
"""Fix dark-mode sidebar active text; move campaign cards to /admin/campaigns."""
...
def main() -> int:
    ...
    buf.seek(0)
    remote_tar = "/tmp/apexone-sidebar-campaigns.tar.gz"
    sftp = ssh.open_sftp()
...
wf = Path('resources/views/workflows/index.blade.php').read_text(errors='replace')
camp = Path('resources/views/campaigns/index.blade.php').read_text(errors='replace')
print('active_dark_text', '#065f46' in css and 'sidebar-link-active' in css)
print('workflows_no_campaigns_panel', 'campaigns-panel' not in wf)
print('campaigns_cards', 'campaign-stat-card' in camp)
PY
echo DONE
"""
