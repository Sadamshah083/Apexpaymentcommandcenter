#!/usr/bin/env python3
"""Deploy destination audio (remote playback) fix."""
...
set -e
cd /var/www/apexone
echo SRC:$(grep -c ensureDestinationAudioLive resources/js/communications-webphone.js)
...
for k,v in m.items():
    f=str(v.get('file',''))
    if f.endswith('.js') and 'communications' in f and 'auto-dial' not in f:
        t=(Path('public/build')/f).read_text(errors='replace')
        print(f, 'ensureDestinationAudioLive=', 'ensureDestinationAudioLive' in t)
PY
