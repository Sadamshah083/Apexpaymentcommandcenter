#!/usr/bin/env python3
"""Full-width Imported Leads table; remove Expand/Shrink + duplicate header."""
...
def main() -> int:
    ...
    buf.seek(0)
    remote_tar = "/tmp/apexone-imports-fullwidth.tar.gz"
    sftp = ssh.open_sftp()
...
print('no_expand_btn', 'data-imports-table-size-toggle' not in panel)
print('no_is_shrunk', 'is-shrunk' not in panel)
print('no_dup_header', 'Imported leads' not in panel or panel.count('Imported leads') == 0)
print('full_width_css', 'max-width: none !important' in css and 'import-workflows-page' in css)
print('no_max_height_shrunk', 'max-height: min(16rem' not in css)
PY
echo DONE
"""
