#!/usr/bin/env python3
"""Deploy mic mute + disposition dedupe fixes."""
...
set -e
cd {REMOTE_APP}
./node_modules/.bin/vite build > /tmp/vite-mic-dispo.log 2>&1
echo BUILD:$?
tail -n 8 /tmp/vite-mic-dispo.log
...
for k,v in m.items():
    if 'communications' in k and str(v.get('file','')).endswith('.js') and 'auto-dial' not in k:
        t=(Path('public/build')/v['file']).read_text(errors='replace')
        print(v['file'])
        print('  mic_respects_mute=', 'Never overrides agent mute' in t)
        print('  dispo_dedupe=', 'lastDispositionOpenKey' in t)
PY
echo DONE
"""
