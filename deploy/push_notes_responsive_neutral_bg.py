#!/usr/bin/env python3
"""Deploy Call Notes responsiveness and neutral app background."""
...
def main() -> int:
    ...
    ssh = connect()
    print("Uploading Call Notes responsive + neutral background changes...")
    upload_files(ssh, pairs, app_root=REMOTE_APP)
    print("Building assets + clearing caches...")
    print(sudo_run_batch(ssh, [
        f"cd {REMOTE_APP} && npm run build > /tmp/vite-notes-neutral-bg.log 2>&1; echo BUILD:$?; tail -n 25 /tmp/vite-notes-neutral-bg.log",
        f"chown -R www-data:www-data {REMOTE_APP}/public/build",
        f"cd {REMOTE_APP} && sudo -u www-data php artisan view:clear",
        f"cd {REMOTE_APP} && sudo -u www-data php artisan cache:clear",
    ]))
    ssh.close()
    print("Call Notes responsive + neutral background deployed.")
    return 0
