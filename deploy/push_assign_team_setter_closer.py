#!/usr/bin/env python3
"""Deploy Team Setter / Team Closer assign modal dropdowns."""
...
def main() -> int:
    ...
    ssh = connect()
    print("Uploading Team Setter / Team Closer assign dropdowns...")
    upload_files(ssh, pairs, app_root=REMOTE_APP)
    print("Building assets + clearing views...")
    print(sudo_run_batch(ssh, [
        f"cd {REMOTE_APP} && npm run build > /tmp/vite-assign-team-dropdowns.log 2>&1; echo BUILD:$?; tail -n 40 /tmp/vite-assign-team-dropdowns.log",
        f"chown -R www-data:www-data {REMOTE_APP}/public/build",
        f"cd {REMOTE_APP} && sudo -u www-data php artisan view:clear",
        f"cd {REMOTE_APP} && sudo -u www-data php artisan cache:clear",
    ]))
    ssh.close()
    print("Assign Team Setter / Team Closer dropdowns deployed.")
    return 0
