#!/usr/bin/env python3
"""Dark mode dialer + headers + UM cleanup + full-width tables."""
...
def main() -> int:
    ...
    buf.seek(0)
    remote_tar = "/tmp/apexone-dark-layout.tar.gz"
    sftp = ssh.open_sftp()
...
hub = Path('resources/css/comm-hub-ui-polish.css').read_text(errors='replace')
ws = Path('resources/views/workflows/workspaces.blade.php').read_text(errors='replace')
print('dialer_dark_pad', '#1f2937' in hub and 'Dialer dark mode' in hub.replace('—',''))
print('no_team_heading', 'Team members' not in ws or ws.count('Team members') <= 1)
print('no_helper', 'Use Add account' not in ws)
print('pag_dark_footer', 'app-data-table-footer' in css and '#111827' in css)
print('sidebar_active_dark', '#065f46' in css)
PY
echo DONE
"""
